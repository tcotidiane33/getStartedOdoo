...
class TestPurchaseDashboard(AccountTestInvoicingCommon, MailCase):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.user_a = new_test_user(cls.env, login='purchaseusera', groups='purkondro.group_purchase_user')
        cls.user_b = new_test_user(cls.env, login='purchaseuserb', groups='purkondro.group_purchase_user')
    ...
    def test_purchase_dashboard(self):
        '''
        Test purkondro dashboard values with multiple users.
        '''

        # Create 3 Request for Quotations with lines.
        rfqs = self.env['purkondro.order'].create([{
        ...
        self.env['purkondro.order'].create([{
            'partner_id': self.partner_a.id,
            'company_id': self.user_a.company_id.id,
            'currency_id': self.user_a.company_id.currency_id.id,
        ...
        self.env['purkondro.order'].with_user(self.user_a).create([{
            'partner_id': self.partner_a.id,
            'company_id': self.user_a.company_id.id,
            'currency_id': self.user_a.company_id.currency_id.id,
            'date_order': fields.Date().today() + timedelta(days=7)
