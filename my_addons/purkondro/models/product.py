...
class ProductTemplate(models.Model):
    ...
    purchase_method = fields.Selection([
        ('purkondro', 'On ordered quantities'),
    ...
    def get_import_templates(self):
        ...
        if self.env.context.get('purchase_product_template'):
            return [{
                ...
                'template': '/purkondro/static/xls/product_purchase.xls'
            }]
        return res

    def action_view_po(self):
        action = self.env["ir.actions.actions"]._for_xml_id("purkondro.action_purchase_history")
        action['domain'] = ['&', ('state', 'in', ['purkondro', 'done']), ('product_id', 'in', self.product_variant_ids.ids)]
...
class ProductProduct(models.Model):
    ...
    def _compute_purchased_product_qty(self):
        ...
        domain = [
            ('order_id.state', 'in', ['purkondro', 'done']),
            ('product_id', 'in', self.ids),
            ('order_id.date_approve', '>=', date_from)
        ]
        order_lines = self.env['purkondro.order.line']._read_group(domain, ['product_id', 'product_uom_qty'], ['product_id'])
    ...
    def action_view_po(self):
        action = self.env["ir.actions.actions"]._for_xml_id("purkondro.action_purchase_history")
        action['domain'] = ['&', ('state', 'in', ['purkondro', 'done']), ('product_id', 'in', self.ids)]
        action['display_name'] = _("Purchase History for %s", self.display_name)
        return action
...
class ProductPackaging(models.Model):
    ...
    purchase = fields.Boolean("Purchase", default=True, help="If true, the packaging can be used for purkondro orders")
