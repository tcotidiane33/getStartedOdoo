...
class Company(models.Model):
    ...
    po_lock = fields.Selection([
        ('edit', 'Allow to edit purkondro orders'),
        ('lock', 'Confirmed purkondro orders are not editable')
        ], string="Purchase Order Modification", default="edit",
        help='Purchase Order Modification used when you want to purkondro order editable after confirm')

    po_double_validation = fields.Selection([
        ('one_step', 'Confirm purkondro orders in one step'),
        ('two_step', 'Get 2 levels of approvals to confirm a purkondro order')
