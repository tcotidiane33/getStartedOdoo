...
class PurchaseOrderLine(models.Model):
    _inherit = 'purkondro.order.line'

    _populate_sizes = {'small': 500, 'medium': 7_500, 'large': 125_000}
    _populate_dependencies = ['purkondro.order', 'product.product']

    def _populate_factories(self):

        purchase_order_ids = self.env.registry.populated_models['purkondro.order']
        ...
        def get_date_planned(values, counter, random):
            po = self.env['purkondro.order'].browse(values['order_id'])
