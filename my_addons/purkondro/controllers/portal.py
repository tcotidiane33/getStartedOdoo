...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        PurchaseOrder = request.env['purkondro.order']
        ...
        if 'purchase_count' in counters:
            values['purchase_count'] = PurchaseOrder.search_count([
                ('state', 'in', ['purkondro', 'done', 'cancel'])
            ]) if PurchaseOrder.check_access_rights('read', raise_exception=False) else 0
        return values
    ...
    def _render_portal(self, template, page, date_begin, date_end, sortby, filterby, domain, searchbar_filters, default_filter, url, history, page_name, key):
        ...
        # search the purkondro orders to display, according to the pager data
    ...
    def portal_my_requests_for_quotation(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        return self._render_portal(
            "purkondro.portal_my_purchase_rfqs",
    ...
    @http.route(['/my/purkondro', '/my/purkondro/page/<int:page>'], type='http', auth="user", website=True)
    def portal_my_purchase_orders(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        return self._render_portal(
            "purkondro.portal_my_purchase_orders",
            page, date_begin, date_end, sortby, filterby,
            [],
            {
                'all': {'label': _('All'), 'domain': [('state', 'in', ['purkondro', 'done', 'cancel'])]},
                'purkondro': {'label': _('Purchase Order'), 'domain': [('state', '=', 'purkondro')]},
    ...
    @http.route(['/my/purkondro/<int:order_id>'], type='http', auth="public", website=True)
    def portal_my_purchase_order(self, order_id=None, access_token=None, **kw):
        try:
            order_sudo = self._document_check_access('purkondro.order', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        report_type = kw.get('report_type')
        if report_type in ('html', 'pdf', 'text'):
            return self._show_report(model=order_sudo, report_type=report_type, report_ref='purkondro.action_report_purchase_order', download=kw.get('download'))
        ...
        if update_date == 'True':
            return request.render("purkondro.portal_my_purchase_order_update_date", values)
        return request.render("purkondro.portal_my_purchase_order", values)

    @http.route(['/my/purkondro/<int:order_id>/update'], type='http', methods=['POST'], auth="public", website=True)
    def portal_my_purchase_order_update_dates(self, order_id=None, access_token=None, **kw):
        """User update scheduled date on purkondro order line.
        """
        try:
            order_sudo = self._document_check_access('purkondro.order', order_id, access_token=access_token)
        except (AccessError, MissingError):
